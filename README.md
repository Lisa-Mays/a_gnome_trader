...
A Discord bot for EverQuest TLP servers that watches the Frostreaver auction feed and pings you the moment an item you want goes up for sale - with the seller, the asking price, and whether it's a deal. Upcoming features will add other TLP servers, but for now it's just Frostreaver.
...
- `/watch add item:<name>` - options: `notify` (channel/DM/both), `exact`, `max_price`, `private`
- `/watch list` / `all` / `remove` / `pause` / `resume` / `clear` / `status`
- `/bonuses` - today's zone bonuses on demand
- `/bonuswatch add zone:<name>` - pick bonus types from a dropdown; `list` / `remove` / `clear` to manage
- `/update` - force a bonus-board refresh
- `/help` - the full list, in Discord
